  ...
  function renderMap() {
    ...
    const order = Object.keys(byCode).filter(c => byCode[c] != null)
      .sort((a, b) => (diverging ? Math.abs(byCode[b]) - Math.abs(byCode[a])
                                 : byCode[b] - byCode[a]));
    /* Twelve rows left the column 518px short of the drawing beside it, which
       is what made the map read as stranded bottom-left in a half-empty panel.
       Twenty-four brings the two columns level to within ~80px, and a ranked
       list of 77 provinces had no reason to stop at twelve anyway. */
    const ranked = order.slice(0, 24);
